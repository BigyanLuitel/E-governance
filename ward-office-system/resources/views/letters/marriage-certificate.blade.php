...
<body>
    ...
    <div class="title">Recommendation for Marriage Registration</div>

    <div class="body-text">
        This is to certify that <strong>{{ $request->form_data['spouse1_name'] ?? 'N/A' }}</strong> and
        <strong>{{ $request->form_data['spouse2_name'] ?? 'N/A' }}</strong> were married on
        <strong>{{ $request->form_data['date_of_marriage'] ?? 'N/A' }}</strong>, within the jurisdiction of
        this Ward Office.
        <br><br>
        Based on the records and verification available with this office, this Ward Office recommends the
        above particulars for registration and issuance of a marriage certificate by the concerned vital
        registration authority.
        <br><br>
        This recommendation is issued upon the request of the applicant, <strong>{{ $request->citizen->name }}</strong>,
        for the purpose stated: <strong>{{ $request->purpose ?? 'Marriage certificate registration' }}</strong>.
    </div>
