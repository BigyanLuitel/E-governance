...
<body>
    ...
    <div class="title">Recommendation for Death Registration</div>

    <div class="body-text">
        This is to certify that <strong>{{ $request->form_data['deceased_name'] ?? 'N/A' }}</strong> passed away on
        <strong>{{ $request->form_data['date_of_death'] ?? 'N/A' }}</strong>, within the jurisdiction of this Ward
        Office.
        The applicant, <strong>{{ $request->citizen->name }}</strong>, has stated their relationship to the
        deceased as <strong>{{ $request->form_data['relation_to_applicant'] ?? 'N/A' }}</strong>.
        <br><br>
        Based on the records and verification available with this office, this Ward Office recommends the
        above particulars for registration and issuance of a death certificate by the concerned vital
        registration authority.
        <br><br>
        This recommendation is issued upon the request of the applicant for the purpose stated:
        <strong>{{ $request->purpose ?? 'Death certificate registration' }}</strong>.
    </div>
